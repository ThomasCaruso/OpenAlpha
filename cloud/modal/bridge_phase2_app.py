...
def verify_zero_shot_benchmark_deployment() -> dict[str, Any]:
    ...
    return {
        ...
        "bootstrap": {
            ...
            "base_resampling_unit": BOOTSTRAP_BASE_RESAMPLING_UNIT,
            "temporal_resampling_unit": BOOTSTRAP_TEMPORAL_RESAMPLING_UNIT,
            ...
            "wraparound": False,
            "block_length_selected_before_execution": True,
            "individual_asset_origin_interval_retained": False,
            "independent_origin_cluster_interval_retained": False,
        },
...
# ======================================================================
# Kronos-base frozen-representation probe
#
# A fourth study, with its own preregistration, artifact namespace,
# run-identifier space, schemas and workers. It reuses the base study's remote
# volume because the two pinned repositories already live there.
#
# Fit and test are separate functions writing separate immutable artifacts. The
# fit publishes its selected hyperparameters before the test partition is
# opened, so "we did not tune on test" is a record rather than a promise.
# ======================================================================


def _probe_runtime(cache_root: Path, source_root: Path):
    """The official runtime, extended with a hidden-state extractor.

    Assembled from the same public loader functions official_runtime uses, in
    the same order: verify all four asset digests, verify the pinned source,
    enter the isolated import context once, load and freeze inside it, and stay
    entered while the caller reads. The difference is what it yields -- an
    extractor exposing decode_s1 and nothing else, so no generation path is
    ...
    def _enter():
        # All four asset digests and the pinned source verified before a weight
        # is loaded.
        digests = verify_official_assets(
            tokenizer_directory=tokenizer_dir,
            model_directory=model_dir,
            tokenizer_spec=PROBE_TOKENIZER_SPEC,
            model_spec=PROBE_MODEL_SPEC,
        )
        source_digests = verify_source_files(source_root)

        with isolated_official_source(source_root) as official:
            # Note the order: the loader returns the tokenizer first.
            ...
            assets = ResolvedDiagnosticAssets(
                tokenizer_repository=PROBE_TOKENIZER_SPEC.repository,
                tokenizer_revision=PROBE_TOKENIZER_SPEC.revision,
                tokenizer_config_sha256=digests.tokenizer_config_sha256,
                tokenizer_weights_sha256=digests.tokenizer_weights_sha256,
                model_repository=PROBE_MODEL_SPEC.repository,
                model_revision=PROBE_MODEL_SPEC.revision,
                ...
                # Same operand order official_runtime uses, so the digest is
                # comparable with the completed studies.
            ...
            class _Runtime:
                def __init__(self) -> None:
                    self.extractor = OfficialHiddenStateExtractor(
                        model=model, tokenizer=tokenizer, device="cuda"
                    )
                    self.assets = assets
                    self.parameter_digest = lambda: parameter_digest(tokenizer, model)
...
@app.function(image=image, secrets=secrets, timeout=CONTROL_TIMEOUT)
def verify_representation_probe_deployment() -> dict[str, Any]:
    ...
    research_root = Path("/root/research/bridge-v0")
    probe_hashes = verify_probe_specification(research_root)
    # Every historical document must still verify.
    base_hashes = verify_base_specification(research_root)
    mini_sealed = verify_locked_hashes(research_root)
    mini_diagnostic = verify_diagnostic_specifications(research_root)

    verify_pinned_pair()
    if PROBE_MODEL_SPEC.revision != KRONOS_BASE_REVISION:
        raise RuntimeError("PROBE_MODEL_REVISION_DRIFT")
    if PROBE_TOKENIZER_SPEC.revision != KRONOS_BASE_TOKENIZER_REVISION:
        raise RuntimeError("PROBE_TOKENIZER_REVISION_DRIFT")
    ...
    return {
        ...
        "probe_specification_hashes": probe_hashes,
        "base_specification_hashes": base_hashes,
        "mini_locked_hashes": mini_sealed,
        "mini_diagnostic_specification_hashes": mini_diagnostic,
        "sealing_commit": SEALING_COMMIT,
        "sealed_at_utc": SEALED_AT_UTC,
        "artifact_namespace": PROBE_ARTIFACT_ROOT,
        "run_id_pattern": "^frp_[0-9a-f]{8,32}$",
        "fit_schema": PROBE_FIT_SCHEMA_VERSION,
        "success_schema": PROBE_SUCCESS_SCHEMA_VERSION,
        "failure_schema": PROBE_FAILURE_SCHEMA_VERSION,
        "model": {
            "repository": PROBE_MODEL_SPEC.repository,
            "revision": PROBE_MODEL_SPEC.revision,
            "weights_sha256": PROBE_MODEL_SPEC.weights_sha256,
        },
        "tokenizer": {
            "repository": PROBE_TOKENIZER_SPEC.repository,
            "revision": PROBE_TOKENIZER_SPEC.revision,
            "weights_sha256": PROBE_TOKENIZER_SPEC.weights_sha256,
        },
        "assets": list(ASSET_PANEL),
        "context_candles": CONTEXT_CANDLES,
        "horizon_candles": HORIZON_CANDLES,
        "stride": STRIDE,
        "context_budget": prove_probe_budget(),
        "partition_geometry": verify_partition_geometry(),
        ...
        "train_validation_origin_offsets": list(TRAIN_VALIDATION_ORIGIN_OFFSETS),
        ...
        "minimum_test_origins_per_asset": MINIMUM_TEST_ORIGINS_PER_ASSET,
        "minimum_test_samples": MINIMUM_TEST_SAMPLES,
        "representation_dimension": REPRESENTATION_DIMENSION,
        "feature_set_dimensions": dict(FEATURE_SET_DIMENSIONS),
        "controls": sorted(CONTROL_BUILDERS),
        "ridge_alpha_grid": list(RIDGE_ALPHA_GRID),
        "logistic_c_grid": list(LOGISTIC_C_GRID),
        "primary_metric": PRIMARY_METRIC,
        "secondary_metrics": list(SECONDARY_METRICS),
        "decision_rules": ["R0", "R1", "R2", "R3"],
        "decision_outcomes": [
            "PROCEED_TO_SUPERVISED_ADAPTATION_DESIGN",
            "PROCEED_TO_CONFIRMATION_ON_FRESH_ORIGINS",
            "STOP_KRONOS_REPRESENTATION_DIRECTION",
            "PROBE_INCONCLUSIVE",
        ],
        "thresholds": PROBE_THRESHOLDS.model_dump(mode="json"),
        "bootstrap": {
            "block_length": BOOTSTRAP_BLOCK_LENGTH,
            "resamples": BOOTSTRAP_RESAMPLES,
            "confidence_level": BOOTSTRAP_CONFIDENCE_LEVEL,
        ...
        "remote_cache_volume": PROBE_CACHE_VOLUME,
        "creates_new_weight_storage": False,
...
def verify_representation_probe_runtime() -> dict[str, Any]:
    """Does the deployed image extract a well-formed hidden state?

    A compatibility probe. It retrieves no market data, fits nothing, evaluates
    no decision rule and authorizes nothing, including this study.
    """
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("PROBE_RUNTIME_NO_CUDA: the probe exercises the deployed GPU path")

    # Synthetic context. No market data is retrieved by this function.
    rows = tuple(
        OfficialRow(
        ...
        for i in range(CONTEXT_CANDLES)
    )
    stamps = tuple(official_stamp(r.session) for r in rows)
    state = fit_context_state(rows)

    with _probe_runtime(cache_root, source_root) as runtime:
        before = runtime.parameter_digest()
        extracted = runtime.extractor.hidden_state(rows, context_stamps=stamps, state=state)
        after = runtime.parameter_digest()
        assets = runtime.assets.model_dump(mode="json")

    base_cache_volume.commit()
    return {
        "schema_version": PROBE_PROBE_SCHEMA_VERSION,
        "outcome": "KRONOS_REPRESENTATION_PROBE_RUNTIME_PASSED",
        "claim_boundary": (
            "RUNTIME COMPATIBILITY PROBE - NOT A RESULT AND NOT EMPIRICAL EVIDENCE"
        ),
        "experiment_id": PROBE_EXPERIMENT_ID,
        "assets": assets,
        "representation_dimension": extracted.dimension,
        "expected_dimension": REPRESENTATION_DIMENSION,
        "sequence_length": extracted.sequence_length,
        "batch_size": extracted.batch_size,
        "encoded_token_count": extracted.encoded_token_count,
        "parameter_sha256_before": before,
        "parameter_sha256_after": after,
        "parameters_unmodified": before == after,
        "environment": {
            "torch_version": torch.__version__,
            "torch_cuda_version": torch.version.cuda,
            "cuda_available": True,
        ...
        "decision_rules_evaluated": False,
        "test_partition_opened": False,
        "authorizes_the_representation_probe": False,
        "authorizes_fine_tuning": False,
...
def fit_frozen_representation_probe(source_commit: str, run_id: str) -> dict[str, Any]:
    """Phase one: extract, select on validation, refit, publish the fit artifact.

    Retrieves only the train/validation window, which ends strictly before the
    sealed test boundary, and refuses if any retrieved session reaches it.
    """
    from pathlib import Path

    from openalpha_bridge.phase2.provider import YahooDailyProvider
    from openalpha_bridge.representation_probe.worker import run_probe_fit_worker
    ...
    result = run_probe_fit_worker(
        store=_build_store(),
        resolve_runtime=lambda: _probe_runtime(cache_root, source_root),
        provider_factory=lambda: YahooDailyProvider(stage="kronos-representation-probe-fit"),
        research_root=Path("/root/research/bridge-v0"),
        source_commit=source_commit,
        deployed_commit=_require_deployed_commit(),
        run_id=run_id,
...
def test_frozen_representation_probe(
    source_commit: str, run_id: str, expected_fit_digest: str
) -> dict[str, Any]:
    """Phase two: verify every gate, then score the untouched test partition.

    expected_fit_digest is required, not optional: the caller must name the
    exact fit artifact this test is entitled to run against, and a mismatch
    fails closed. The eligibility gate refuses to proceed until the retrieved
    calendar supplies the declared minimum, so an early invocation reads nothing
    it should not.
    """
    from pathlib import Path

    from openalpha_bridge.phase2.provider import YahooDailyProvider
    from openalpha_bridge.representation_probe.worker import run_probe_test_worker
    ...
    result = run_probe_test_worker(
        store=_build_store(),
        resolve_runtime=lambda: _probe_runtime(cache_root, source_root),
        provider_factory=lambda: YahooDailyProvider(stage="kronos-representation-probe-test"),
        research_root=Path("/root/research/bridge-v0"),
        source_commit=source_commit,
        deployed_commit=_require_deployed_commit(),
        run_id=run_id,
...
@app.function(image=image, secrets=secrets, timeout=CONTROL_TIMEOUT)
def inventory_representation_probe_artifacts() -> dict[str, Any]:
    """Report what exists under the probe artifact root. Read-only."""
    from openalpha_bridge.representation_probe.spec import (
        PROBE_ARTIFACT_ROOT,
    ...
    _register_secrets()
    store = _build_store()
    prefix = f"{PROBE_ARTIFACT_ROOT}/runs/"
    records = []
    runs: set[str] = set()
        ...
        records.append(
            {
                "key": key,
                "run_id": run_id,
                "run_id_is_well_formed": bool(PROBE_RUN_ID_PATTERN.fullmatch(run_id)),
                "phase": (
                    "fit"
                    if tail == PROBE_FIT_OBJECT_NAME
                    else ("test" if tail == PROBE_TEST_OBJECT_NAME else "unknown")
                ),
            }
        )
    return {
        "artifact_root": PROBE_ARTIFACT_ROOT,
        "records": records,
        "run_count": len(runs),
        "read_only": True,
        "deletion_supported": False,
        "weights_loaded": False,
        "foreign_roots_inspected": False,
        "deployed_commit": _require_deployed_commit(),
