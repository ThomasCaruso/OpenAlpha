...
from openalpha_kronos.model.normalization import ClippingReport, NormalizationState
from openalpha_kronos.studies.provenance import MINI_RUN_ID_PATTERN, verify_locked_hashes
...
from openalpha_kronos.studies.zero_shot import aggregation as aggregation_module
from openalpha_kronos.studies.zero_shot import baselines as baselines_module
...
from openalpha_kronos.studies.zero_shot.artifact import (
    ZERO_SHOT_FAILURE_CODE,
    ZERO_SHOT_OPERATIONAL_FAILURE_CODE,
    ZERO_SHOT_OPERATIONAL_FAILURE_MESSAGE,
    ZERO_SHOT_SUCCESS_CODE,
)
from openalpha_kronos.studies.zero_shot.baselines import (
    BASELINE_IDS,
    PRIMARY_BASELINE_ID,
    build_baselines,
)
from openalpha_kronos.studies.zero_shot.decision import (
    AssetSupport,
...
from openalpha_kronos.studies.zero_shot.inventory import (
    FOREIGN_ROOTS,
    inventory_zero_shot_artifacts,
)
from openalpha_kronos.studies.zero_shot.invocation import ZeroShotInvocation
from openalpha_kronos.studies.zero_shot.origins import (
    ORIGIN_INDEX_OFFSETS,
    resolve_origins,
    verify_origin_policy,
)
from openalpha_kronos.studies.zero_shot.runner import verify_cross_asset_alignment
...
from openalpha_kronos.studies.zero_shot.worker import (
...
def test_a_wrongly_sized_context_is_refused() -> None:
    from openalpha_kronos.studies.zero_shot.runner import _window_series
...
def test_a_third_temperature_is_refused() -> None:
    from openalpha_kronos.studies.zero_shot.runner import _require_configurations
    from openalpha_kronos.studies.zero_shot.spec import SamplingConfiguration
...
def test_the_seed_set_is_preregistered_and_shared_by_every_cell() -> None:
    from openalpha_kronos.studies.zero_shot.runner import _require_seeds
...
def test_the_block_length_is_derived_from_the_benchmark_geometry() -> None:
    from openalpha_kronos.studies.zero_shot.origins import ORIGIN_STRIDE
...
def test_z1_reads_only_the_moving_block_interval() -> None:
    from openalpha_kronos.studies.zero_shot.spec import ZERO_SHOT_THRESHOLDS
...
def _origin_result(asset: str, ordinal: int, *, shift_target: int = 0, shift_context: int = 0):
    from openalpha_kronos.studies.zero_shot.runner import OriginResult
...
def test_the_benchmark_reuses_the_existing_volume_and_creates_no_new_one() -> None:
    ...
    from openalpha_kronos.studies.zero_shot.spec import ZERO_SHOT_CACHE_VOLUME
