...
from openalpha_kronos.studies.structural_validity.mini.safe_logging import STAGES, StageTracker
...
def _expect_unexpected_failure(
    ...
    *,
    ...
    assert not isinstance(excinfo.value, ResearchFailureError)
...
def test_a_rejected_invocation_stays_typed_and_is_never_unexpected_logged() -> None:
    """A commit mismatch escapes publication as a ResearchFailureError."""
...
def test_the_worker_wraps_the_whole_publication() -> None:
    ...
    outer = source.index("except ResearchFailureError:", published)
