...
from openalpha_kronos.studies.structural_validity.mini import safe_logging
...
def test_the_worker_logs_only_unexpected_failures() -> None:
    ...
    assert "except ResearchFailureError:" in source
    assert "log_operational_failure(" in source
    assert source.index("except ResearchFailureError:") < source.index("log_operational_failure(")


def test_a_typed_failure_needs_no_sanitising() -> None:
    assert issubclass(ResearchFailureError, Exception)
